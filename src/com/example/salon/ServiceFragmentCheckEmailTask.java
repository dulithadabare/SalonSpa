...
public class ServiceFragmentCheckEmailTask extends AsyncTask<String,Integer,String>{

	
	ServiceSearchFragment activity;
	 ServiceFragmentCheckEmailTask(ServiceSearchFragment activity2){
	...
	//	ServiceSearchFragment.ServiceAdapter=result;
		//ServiceSearchFragment.list.setAdapter(result);
		 if(result.equals("0")){
      	  new ServiceFragmentGetUserDetails(activity, ActivityMain.selected_account_name, ActivityMain.SCOPE,
      			  ActivityMain.REQUEST_CODE_RECOVER_FROM_AUTH_ERROR).execute();
      	 
      	  
        }else{
      	  ActivityMain.salonspa_user_id=result;
      	  new ServiceFragmentBookingTask(activity).execute();
        }
	...
	protected void onPreExecute() {
		...
		ServiceSearchFragment.mDialog.setMessage("Booking in progress");
		ServiceSearchFragment.mDialog.show();
