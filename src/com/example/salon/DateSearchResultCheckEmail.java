...
public class DateSearchResultCheckEmail extends AsyncTask<String,Integer,String>{

	
	DateSearchResult activity;
	DateSearchResultCheckEmail(DateSearchResult activity2){
	...
	//	ServiceSearchFragment.ServiceAdapter=result;
		//ServiceSearchFragment.list.setAdapter(result);
		 if(result.equals("0")){
      	  new DateSearchResultGetUserDetails(activity, ActivityMain.selected_account_name, ActivityMain.SCOPE,
      			  ActivityMain.REQUEST_CODE_RECOVER_FROM_AUTH_ERROR).execute();
      	 
      	  
        }else{
      	  ActivityMain.salonspa_user_id=result;
      	  new DateSearchResultBookingTask(activity).execute();
        }
	...
	protected void onPreExecute() {
		...
		DateSearchResult.mDialog.setMessage("Booking in progress");
		DateSearchResult.mDialog.show();
