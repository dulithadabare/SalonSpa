...
public class ServiceSearchCheckEmailTask extends AsyncTask<String,Integer,String>{

	
	ServiceSearch activity;
	 ServiceSearchCheckEmailTask(ServiceSearch activity2){
	...
	//	ServiceSearchFragment.ServiceAdapter=result;
		//ServiceSearchFragment.list.setAdapter(result);
		 if(result.equals("0")){
      	  new ServiceSearchGetUserDetails(activity, ActivityMain.selected_account_name, ActivityMain.SCOPE,
      			  ActivityMain.REQUEST_CODE_RECOVER_FROM_AUTH_ERROR).execute();
      	 
      	  
        }else{
      	  ActivityMain.salonspa_user_id=result;
      	  new ServiceSearchBookingTask(activity).execute();
        }
	...
	protected void onPreExecute() {
		...
		ServiceSearch.mDialog.setMessage("Booking in progress");
		ServiceSearch.mDialog.show();
